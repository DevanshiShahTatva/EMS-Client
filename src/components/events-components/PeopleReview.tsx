...
import { FeedbackItem } from '@/app/events/types';
...
const ReviewCard = ({ user, updatedAt, rating, description }: FeedbackItem) => (
...
const ReviewsRightSection = ({ reviews, averageRating, eventName }: { eventName: string; averageRating: string; reviews: FeedbackItem[] }) => {
        ...
        <div className="absolute top-1/2 right-4 transform -translate-y-1/2 flex -space-x-4">
          {reviews.slice(0, 5).map((review, i) => {
            return review.user?.profileimage ?
              ...
              /> :
              <div key={`${i + 1}`} className={`z-${i + 1} border flex justify-center items-center h-10 w-10 rounded-full bg-white text-indigo-600 font-bold relative`}>
