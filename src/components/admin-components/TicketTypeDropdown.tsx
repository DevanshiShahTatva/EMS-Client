...
import {
  ...
  ITicketTypesResp,
} from "@/app/admin/dropdowns/types";
import {
  getSearchResults,
  initialTicketTypeFormValues,
} from "@/app/admin/dropdowns/helper";
...
function TicketTypeDropdown() {
  ...
  const [usedInEvents, setUsedInEvents] = useState<
    { _id: string; title: string; endDateTime: string }[]
  >([]);
  const [isModalOpen, setModalOpen] = useState(false);
  const [initialValues, setInitialValues] = useState<ITicketTypeFormValues>(
    initialTicketTypeFormValues
  );
  ...
  const createTicketType = useCallback(
    async (form: ITicketTypeFormValues) => {
  ...
  const tableheaders: {
    ...
    render?: (item: ITicketType) => string;
  }[] = [
    ...
    { header: "Description", key: "description" },
    {
      header: "Status",
      key: "isUsed",
      render: (item: ITicketType) => (item.isUsed ? "In Use" : "Not Used"),
    },
  ];
  ...
  const tableActions: Action<ITicketType>[] = [
    {
      icon: (
        <PencilSquareIcon className="h-5 w-5 text-blue-500 hover:text-blue-700 cursor-pointer" />
      ),
